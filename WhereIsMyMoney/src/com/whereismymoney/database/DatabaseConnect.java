...
public class DatabaseConnect implements Idatabase {
	...
	public Document getAllAccounts(String username) {
		try {
			String address = buffer.append(ROOT_ADDRESS).append(GET_ALL_ACCOUNTS).toString();
	...
	public  Document createAccount(String username, String account_display_name,
		...
		try {
			String address = buffer.append(ROOT_ADDRESS).append(CREATE_ACCOUNT).toString();
            Document doc = Jsoup.connect(address)
            		.data("username", username)
	...
	public Document login(String username, String password) {
		try {
			String address = buffer.append(ROOT_ADDRESS).append(LOGIN).toString();
            Document doc = Jsoup.connect(address)
                    .data("username", username)
	...
	public Document register(String username, String first_name,
		...
		try {
			String address = buffer.append(ROOT_ADDRESS).append(REGISTER).toString();
            Document doc = Jsoup.connect(address)
                    .data("username", username)
                    .data("password", password)
	...
	public Document generateSpendingCategoryReport(String username, Date start,
			Date end) {
		try {
			String address = buffer.append(ROOT_ADDRESS).append(GENERATE_SPENDING_CATEGORY_REPORT).toString();
            Document doc = Jsoup.connect(address)
                    .data("username", username)
	...
	public Document newDeposit(String source, double amount,
		...
		try {
			String address = buffer.append(ROOT_ADDRESS).append(CREATE_DEPOSIT).toString();
            Document doc = Jsoup.connect(address)
	...
	public Document newWithdrawal(String reason, String expenseCategory,
		...
		try {
			String address = buffer.append(ROOT_ADDRESS).append(CREATE_WITHDRAWAL).toString();
            Document doc = Jsoup.connect(address)
            		.data("username", CurrentUser.getCurrentUser().getUserName())
    				.data("account_name", CurrentAccount.getCurrentAccount().getAccountName())
    				.data("amount", Double.toString(amount))
